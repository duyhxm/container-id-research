...
class DetectionConfig:
    ...
    def validate(self) -> bool:
        ...
        # Validate model
        valid_architectures = ['yolov11n', 'yolov11s', 'yolov11m', 
                               'yolov11l', 'yolov11x']
        ...
        if not 0 <= self.augmentation.hsv_h <= 1:
            raise ValueError(
                f"hsv_h must be in [0, 1], got {self.augmentation.hsv_h}"
            )
        if not 0 <= self.augmentation.hsv_s <= 1:
            raise ValueError(
                f"hsv_s must be in [0, 1], got {self.augmentation.hsv_s}"
            )
        if not 0 <= self.augmentation.hsv_v <= 1:
            raise ValueError(
                f"hsv_v must be in [0, 1], got {self.augmentation.hsv_v}"
            )
