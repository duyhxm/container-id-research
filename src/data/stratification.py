...
class StratifiedSplitter:
        ...
        for img in coco_data['images']:
            ...
            for key, values in aggregated_attrs.items():
                ...
                # Occluded boolean field (alternative field name)
                    ...
                    if True in values or 'True' in values or any(v for v in values if v is True):
                        attr_list.append('occluded')
    ...
    def _augment_singleton(
        ...
        try:
            ...
            print(f"    ⚠ Warning: Augmentation failed for {singleton_img['file_name']}: {e}")
            return None
    ...
    def run(
        ...
        output_dir: Path,
        augmented_images_dir: Path = None
        ...
        images_with_ann = [img for img in coco_data['images'] if img['id'] in img_ids_with_ann]
        ...
        if singletons:
            ...
            try:
                ...
                print(f"  ✓ Created {len(singleton_train_imgs)} augmented images for TRAIN")
                print(f"  ✓ Assigned {len(singleton_test_imgs)} original singletons to TEST")
                ...
                print(f"  Impact: Training set will have {len(non_singletons)} samples instead of {len(non_singletons) + len(singletons)}")
        ...
        print(f"  ✓ Train: {len(train_imgs)} images ({len(singleton_train_imgs)} augmented singletons)")
        print(f"  ✓ Val: {len(val_imgs)} images")
        print(f"  ✓ Test: {len(test_imgs)} images ({len(singleton_test_imgs)} original singletons)")
