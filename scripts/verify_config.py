...
def verify_configuration(config_path: Path = Path("params.yaml")) -> bool:
    ...
    if det["model"]["architecture"] == "yolo11s":
        print("  ✓ Model: yolo11s (YOLOv11-Small)")
        checks.append(True)
    else:
        print(f"  ✗ Model: {det['model']['architecture']} (Expected: yolo11s)")
        checks.append(False)
    ...
    if det["training"]["warmup_epochs"] >= 5:
        ...
        print(
            f"  ⚠ Warmup epochs: {det['training']['warmup_epochs']} (Recommended: 5 for batch 32)"
        )
        checks.append(True)  # Warning but acceptable
